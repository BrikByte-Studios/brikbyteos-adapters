...
// Normalizer converts adapter-private k6 parse output into the canonical
// normalized result contract shared across adapters.
type Normalizer struct{}

// Normalize converts a k6 ParseResult plus raw execution truth into canonical JSON.
func (Normalizer) Normalize(input ParseResult, raw sdk.RawExecution) sdk.NormalizedResult {
	...
	thresholdFailedCount := countFailedThresholds(input.Thresholds)
	totalThresholds := len(input.Thresholds)

	payload.Summary.Total = totalThresholds
	payload.Summary.Passed = totalThresholds - thresholdFailedCount
	payload.Summary.Failed = thresholdFailedCount
	payload.Summary.Skipped = 0
	payload.Summary.Status = canonicalStatusFromThresholds(thresholdFailedCount)
...
func countFailedThresholds(thresholds []ParsedThresholdSummary) int {
	count := 0
	for _, threshold := range thresholds {
		if threshold.Status == ThresholdStatusFail {
			count++
		}
	}
	return count
...
func parserErrorMessage(err *ParserError) string {
	if err == nil || strings.TrimSpace(err.Message) == "" {
		return "failed to parse k6 tool output"
	}
	return err.Message
}
...
func mustMarshalNormalized(payload normalizedPayload) sdk.NormalizedResult {
	...
	if err != nil {
		fallback := `{"schema_version":"0.1","adapter":{"name":"k6","type":"performance","version":"UNKNOWN"},"execution":{"status":"unknown","duration_ms":0},"summary":{"status":"unknown","total":0,"passed":0,"failed":0,"skipped":0},"evidence":{"complete":false,"issues":[{"code":"NORMALIZATION_FAILURE","message":"failed to marshal normalized k6 payload"}]},"artifacts":{"raw_stdout_path":"","raw_stderr_path":"","raw_tool_output_path":""},"extensions":{"adapter_specific":{}}}`
