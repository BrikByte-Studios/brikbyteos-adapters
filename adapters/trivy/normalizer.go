...
// Normalizer converts adapter-private Trivy parse output into the canonical
// normalized result contract shared across adapters.
type Normalizer struct{}

// Normalize converts a Trivy ParseResult plus raw execution truth into canonical JSON.
func (Normalizer) Normalize(input ParseResult, raw sdk.RawExecution) sdk.NormalizedResult {
	...
	totalFindings := input.Summary.VulnerabilityTotal
	failedFindings := input.Summary.SeverityCounts.Critical + input.Summary.SeverityCounts.High
	passedFindings := totalFindings - failedFindings
	if passedFindings < 0 {
		passedFindings = 0
	}

	payload.Summary.Total = totalFindings
	payload.Summary.Passed = passedFindings
	payload.Summary.Failed = failedFindings
...
func canonicalStatusFromSummary(summary ParsedSummary) string {
	if summary.SeverityCounts.Critical > 0 || summary.SeverityCounts.High > 0 {
		return "failed"
	}
...
func parserErrorMessage(err *ParserError) string {
	if err == nil || strings.TrimSpace(err.Message) == "" {
		return "failed to parse trivy tool output"
	}
	return err.Message
}
...
func mustMarshalNormalized(payload normalizedPayload) sdk.NormalizedResult {
	...
	if err != nil {
		fallback := `{"schema_version":"0.1","adapter":{"name":"trivy","type":"security","version":"UNKNOWN"},"execution":{"status":"unknown","duration_ms":0},"summary":{"status":"unknown","total":0,"passed":0,"failed":0,"skipped":0},"evidence":{"complete":false,"issues":[{"code":"NORMALIZATION_FAILURE","message":"failed to marshal normalized trivy payload"}]},"artifacts":{"raw_stdout_path":"","raw_stderr_path":"","raw_tool_output_path":""},"extensions":{"adapter_specific":{}}}`
